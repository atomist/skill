...
export interface PersistChangesArgs {
	 ...
	 * returns a string, that string is used as the commit message. If
	 * it returned `undefined`, committing is deferred until a
	 * subsequent editor returns a string. If there are uncommitted
	 * changes when all editors have been run, [[message]] is used as
	 * the commit message for the remaining changes.
	 */
	editors: CommitEditor[];
	/**
	 * Commit message. If [[editor]] returns a string, that takes
	 * precedence as the commit message. If [[message]] is not
